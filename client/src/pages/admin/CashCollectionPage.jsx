...
export const CashCollectionPage = () => {
  ...
  const [dateRangePreset, setDateRangePreset] = useState('all'); // all, today, yesterday, this_month, custom
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success(res.data.message || 'Collection entry deleted successfully.');
  ...
  const handleExportCSV = async () => {
    try {
      ...
      if (res.data.success && res.data.collections) {
        ...
        if (rows.length === 0) {
          toast.error('No cash collection records to export.', { id: 'csv-export' });
          return;
        }
        ...
        link.setAttribute('download', `Cash_Collections_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const topWorker = stats.workerBreakdown && stats.workerBreakdown.length > 0 ? stats.workerBreakdown[0] : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                Admin Panel • Track and tally daily cash & digital payments received from workers
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 px-4 py-2.5 text-xs font-bold text-white shadow-md shadow-emerald-500/25 transition-all active:scale-[0.98]"
          >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-emerald-200/80 dark:border-emerald-500/20 bg-gradient-to-br from-emerald-50/80 via-white to-teal-50/30 dark:from-emerald-950/30 dark:via-slate-900 dark:to-teal-950/20 p-5 shadow-xs">
          ...
          <div className="mt-3">
            ...
            <p className="mt-1 text-xs text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
              <span>{stats.todayCount || 0} collection entries today</span>
            </p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-purple-200/80 dark:border-purple-500/20 bg-gradient-to-br from-purple-50/80 via-white to-fuchsia-50/30 dark:from-purple-950/30 dark:via-slate-900 dark:to-fuchsia-950/20 p-5 shadow-xs">
          ...
          <div className="mt-3">
            <p className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">
              ₹{(stats.totalFilteredAmount || 0).toLocaleString('en-IN')}
            </p>
            <p className="mt-1 text-xs text-purple-600 dark:text-purple-400 font-semibold">
              Across {totalCount} collection records
            </p>
          </div>
        </div>

        {/* Top Worker Collector */}
        <div className="relative overflow-hidden rounded-2xl border border-amber-200/80 dark:border-amber-500/20 bg-gradient-to-br from-amber-50/80 via-white to-orange-50/30 dark:from-amber-950/30 dark:via-slate-900 dark:to-orange-950/20 p-5 shadow-xs">
          <div className="flex items-center justify-between">
          ...
          <div className="mt-3">
            <p className="text-lg font-bold text-slate-900 dark:text-white truncate">
              {topWorker ? topWorker.employeeName : 'No data'}
            </p>
            <p className="mt-1 text-xs text-amber-600 dark:text-amber-400 font-bold">
              {topWorker ? `₹${(topWorker.totalAmount || 0).toLocaleString('en-IN')} collected` : '0 collections'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-xs space-y-3.5">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search worker, receipt, ref..."
              value={search}
              onChange={(e) => {
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          {/* Custom Date Pickers if custom or set */}
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
            ...
            <h3 className="text-base font-bold text-slate-800 dark:text-slate-200">No Cash Collections Found</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-sm mt-1">
              No cash collection entries match your filters. Click below to record a new collection from a worker.
            </p>
            <button
              onClick={() => {
                setEditingItem(null);
              ...
              className="mt-4 flex items-center gap-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-xs font-bold text-white shadow-sm"
            >
              <Plus className="h-4 w-4" />
            ...
            <table className="w-full text-left text-xs text-slate-600 dark:text-slate-300">
              <thead className="bg-slate-50 dark:bg-slate-800/60 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  <th className="py-3.5 px-4">Worker / Sales Rep</th>
                ...
                {collections.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-2.5">
                          <div
                            ...
                            style={{
                              backgroundColor: item.employeeId?.avatarColor || '#4f46e5',
                            }}
                          >
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400">
                              {item.employeeId?.phone || item.employeeId?.email || 'Worker'}
                            </p>
                          </div>
                        </div>
                      </td>
                      ...
                      {/* Amount Collected */}
                      <td className="py-3 px-4 whitespace-nowrap text-right font-black text-sm text-emerald-600 dark:text-emerald-400">
      ...
      {/* Add / Edit Cash Collection Modal */}
      {isAddModalOpen && (
        <CashCollectionModal
      ...
      {deletingItem && (
        <ConfirmModal
          ...
          title="Delete Cash Collection Entry?"
          message={`Are you sure you want to delete the cash collection of ₹${(
            deletingItem.amount || 0
          ).toLocaleString('en-IN')} from ${deletingItem.employeeName} on ${new Date(
            deletingItem.date
